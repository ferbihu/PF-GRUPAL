...
export function addUser({ name, email, password }) {
  return function (dispatch) {
    ...
    axios
      .post(`${REACT_APP_BACK_BASE_URL}/auth/new`, user)
      .then((res) =>
        dispatch({
          type: "ADD_USER",
          payload: res.data,
...
export function postAprobation(payload, userId) {
  ...
  return async function (dispatch) {
    ...
    const res = await axios.post(
      `${REACT_APP_BACK_BASE_URL}/safe_place`,
      { ...payload, userId },
      config
    );
    return {
...
export function login({ email, password }) {
  return function (dispatch) {
    ...
    return axios
      .post(`${REACT_APP_BACK_BASE_URL}/auth/login`, user)
      .then((res) => {
...
export function deleteSafePlace(payload, userId) {
  return async function (dispatch) {
    const json = await axios.get(
      `${REACT_APP_BACK_BASE_URL}/delete_safe_place`,
      { ...payload, userId }
    );
    return dispatch({
